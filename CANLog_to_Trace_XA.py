        ...
        with open(file_path, 'r') as infile, open(output_file, 'w') as outfile:
            ...
            for line in infile:
                ...
                try:
                    ...
                    relative_time = (timestamp - first_timestamp) *1000  # Calculate relative time
                    time_offset = f"{relative_time:.3f}".rjust(8)  # Right-align to match format

                    can_id, data = parts[2].split('#')
                    length = len(data) // 2
                    bus = parts[1][-1]  # Assume the bus is the last character of the bus string
                    data_bytes = " ".join(data[i:i+2] for i in range(0, len(data), 2))
                    data_bytes = data_bytes.ljust(21)  # Adjust padding to shift data

                    # Write the line in TRC format with proper formatting
                    outfile.write(
                        f"{str(message_number).rjust(8)}      {time_offset} DT {bus}  {can_id} Rx -  {length:<4} {data_bytes}\n"
                    )
                    message_number += 1
